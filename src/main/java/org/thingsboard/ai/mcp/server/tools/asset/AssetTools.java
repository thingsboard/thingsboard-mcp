...
public class AssetTools implements McpTools {
    ...
    public String getAssetsByIds(@ToolParam(description = "A string of assets ids, separated by comma ','") @NotBlank String assetIds) {
        List<AssetId> assetIdList = Arrays.stream(assetIds.split(",")).map(UUID::fromString).map(AssetId::new).toList();
        return JacksonUtil.toString(clientService.getClient().getAssetsByIds(assetIdList));
    }
